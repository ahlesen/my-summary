...
def train(train_texts, train_labels, mode='br', gram_range=(1, 2), alpha=1e-4,lr=1e-2,n_epoch=30000,optim='adam', early_stop= 500):


    train_texts = preprocess(train_texts, stop_words=True, punctuation=True)
    train_texts_tok = tokenization_gram(train_texts)

    vocab_1_2 = make_vocab_ngram(train_texts_tok,gram_range=gram_range)

    X_train, vocabulary_1_2 = to_mtx(train_texts_tok, vocab_1_2,with_vocab=True)
    ...
    params = [W, vocab_1_2, vocabulary_1_2]
    return params
